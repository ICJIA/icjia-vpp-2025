...
| Date | Tool | Result |
|------|------|--------|
| 2026-09-16 | WCAG 2.1 AA evaluation (WCAG-EM, all 50 success criteria, every page, both themes; Playwright + axe-core 4.13.0 + pixel measurement) | Web pages partially conformant: 11 failures across 9 success criteria (1.1.1, 1.4.3, 1.4.10, 1.4.11, 1.4.13, 2.4.4, 2.5.2, 2.5.3, 3.1.1). All 11 fixed and re-measured in [2.3.1]. The plan PDF (untagged) was found not to conform and is not part of that release |
...
### Fixed
- **Home: the plan-cover link was named for a picture it is not** (1.1.1, 2.4.4). Its `aria-label` and alt text read "Youth violence prevention summit participants collaborating on community safety initiatives"; the image is the plan's front cover, and the link goes to the download page. The `aria-label` is removed and the alt text now names the link: "Front cover of the Statewide Violence Prevention Plan for Illinois: 2025-2029. Opens the download page." (`data/home.js`, `HomeHero.astro`).
- **Goals and Recommendations: Figure 1's text existed only as pixels** (1.1.1). The figure defines each level of the social-ecological model with examples, and none of it was on the site as text (0 of 10 distinctive phrases on any page). A closed-by-default "Text version of Figure 1" disclosure (`<details>`) now follows the figure with every word in it: each of the four levels as a heading with its two points in a nested list, then the four circle labels. The figure's alt text is shortened to "Where Violence Prevention Efforts Occur: the social-ecological model (text version below)". The transcription was read from the 8001×4500 source image and checked against tesseract OCR of the same image; the plan PDF has no text layer for the figure. The text now also reaches site search and the JSON and YAML exports, which regenerate from the chapter (`goals-and-recommendations.mdx`; summary styles in `global.css`).
- **References: 868px wide at 320px** (1.4.10). Five URLs used as link text are single unbreakable words. Reference links now take `overflow-wrap: anywhere`, which breaks a link only when it cannot fit its line (`global.css`).
- **Citation tooltips on Executive Summary, Public Health Approach and Goals and Recommendations** (`scripts/references.js`):
  - **1.4.10:** at 320px, 5 of 15 tooltips widened the page to 388–416px and were cut off at the edge. The width is now capped at the viewport less 16px each side, long URLs may break anywhere, and the box is clamped inside the viewport and repositioned if the viewport's width changes while one is open.
  - **1.4.13:** a tooltip opened by hovering ignored Escape and sat over the paragraph. Escape is now handled at document level and closes any open tooltip, however it opened. It stays open while the citation or the tooltip is hovered, or while the citation has keyboard focus or was clicked; a keyboard-opened tooltip used to close when the mouse merely passed over the citation.
  - **2.5.2:** it opened on `touchstart`, stayed up for 4 seconds after a finger slid off, and a mouse press opened it through the focus the press gave the citation. The touch handler and its timer are gone; pointers open it on `click` (the up-event), and focus that arrives during a press waits for the click.
  - **Focused citation kept visible** (the evaluation's advisory A1): the tooltip is now sized before it is measured and placed wholly above or below the citation. Citations covered by their own tooltip: 2 → 0 at 1280px, 1 → 0 at 768px, 4 → 0 at 375px.
- **Footer: the inline ICJIA link was set apart only by a faint underline** (1.4.11): 2.01:1 dark and 1.62:1 light against the footer. The underline is now solid in the link colour, 9.90:1 and 5.79:1, and thickens to 2px on hover and focus (`Footer.astro`).
- **Theme switch contrast** (1.4.11). In the light theme at 480px and narrower, where its text label is hidden, the track was 1.47:1 against the pill, the knob 1.62:1 against the track and the sun icon 2.62:1. With the track at `on-surface/70` and the sun at `#c2410c` they measure 5.27, 5.80 and 4.71:1 there, and at least 4.74, 5.80 and 3.81:1 in every light-theme width and hover state. In the dark theme the white knob on the light-blue track was 1.80:1; the knob is now the dark background colour, 9.90:1 (`ThemeToggle.astro`).
- **Theme switch name** (2.5.3). In the light theme it showed "Light" but was named "Toggle dark mode", so "click Light" did nothing for a voice-control user. The visible label is now always "Dark" and the name "Dark mode", with on/off in `aria-checked`. "Dark mode" was tried as the visible label and rejected: it wraps "READ THE PLAN" to three lines at 960–1010px.
- **Download: "Read Online" hover text below 4.5:1 in the light theme** (1.4.3): 4.44:1 in pixels, 4.49:1 by axe-core. Its light-theme text is now emerald-800 `#065f46`: 6.23:1 hovered, active and focused-and-hovered, 7.23:1 at rest (`download.astro`).
- **`/plan/` served a meta-refresh page with no `<html lang>`** (3.1.1). It is now a real 301 to `/plan/front-cover/` in `public/_redirects`, for both `/plan` and `/plan/`, and `src/pages/plan/index.astro` is deleted, as the site documentation's traps section suggested. `/plan` now takes one hop instead of two. The `netlify.toml` note and the sitemap-filter comment in `astro.config.ts` point to the new rule.

### How each fix was verified
- **Names (F1, F9):** Chrome's accessibility tree. The cover link is named by the new alt text in both themes at 1280 and 375px, with no `aria-label`. The switch is "Dark mode" and contains its visible label "Dark" in both themes, in both states, in the header (1280px) and the drawer (600px); `aria-checked` follows the theme; axe-core's `label-content-name-mismatch` goes from 1 violation to 0.
- **Figure 1 (F2):** every line of the figure appears in the text version in order — the title, each level's heading (Societal, Community, Relational, Individual) and its two points, then the four circle labels — checked against the 8001×4500 source image; 10 of 10 test phrases are now on the Goals page and in the JSON export (0 before). The disclosure is announced collapsed, opens and closes with Enter and Space, and has a 44px target with a 2px primary focus outline.
- **Reflow (F3, F4):** `scrollWidth` at 320×640 and 320×256 on all 18 URLs in both themes: References 868 → 320, nothing else over 320. With each of the 15 tooltips opened by focus (both themes), by click and by tap, the widest page goes from 416 to 320px; opened by focus, every tooltip sits within 16–304px, also at 320×256 and after resizing from 1280 to 320px with one open. At 1280px all nine reference links are unchanged, and at 320px the four that already fitted keep the same line breaks.
- **Tooltip behaviour (F5, F10):** hover, then Escape with the pointer on the citation or on the tooltip: visible → hidden, and it stays hidden; it stays open over 5 seconds of hover and 3 seconds on the tooltip itself. Keyboard: Tab opens, Escape closes, Enter reopens, Tab away closes. Touch (CDP emulation, 390×844): after touchStart alone visible → hidden; slide 150px off and lift, visible → hidden; a tap opens on release and stays open (it used to close after 4 seconds) until a tap elsewhere. Mouse, with the hover tooltip dismissed: a press without release leaves it closed, the release opens it, and pressing, dragging off and releasing leaves it closed.
- **Contrast (F6, F7, F8):** computed colours and rendered pixels. The footer underline is a continuous 308px row at 9.90:1 and 5.79:1. The switch's track, knob and icon were measured in both themes at 1280, 600 and 375px, at rest and hovered. "Read Online" was measured at rest, hovered, active, keyboard-focused and focused-and-hovered in both themes, with axe-core passing each light-theme state.
- **`/plan/` (F11):** `/plan/` 200 (a 360-byte stub with no `<html>` or `lang`) and `/plan` 301 → `/plan/` become 301 → `/plan/front-cover/` for both, on a local server applying `_redirects` and `_headers` and again under Netlify CLI 26.1.0 `netlify dev`. The destination page has `lang="en"`.
- **No regressions:** axe-core 4.13.0 WCAG 2.1 A/AA on all 18 URLs × both themes × 1280 and 375px: 0 violations (72 runs), and 0 in 12 more runs with Figure 1's text version open, a citation tooltip open, the drawer open and Read Online hovered. Keyboard: both header menus open with Enter, take Tab and close with Escape; the drawer opens with Enter, keeps focus inside and returns it to the menu button on Escape; search announces "12 results found" and Tab reaches the results; the switch toggles with Space and Enter. The desktop header wraps exactly as in 2.3.0 from 960 to 1920px. No CSP violations or page errors in any run.

### Not in this release
- The accessibility statement and the plan PDF, which the evaluation also assessed, are being handled separately.

Verified: `pnpm build` clean, 17 pages (the `/plan/` stub is no longer built) and 14 sitemap URLs. The JSON and YAML exports regenerated with Figure 1's text in the Goals chapter (39,515 → 41,246 characters).
