...
const config = {
  ...
  publicReportsDir: path.resolve(__dirname, "../public/reports"),
...
function saveReport(report, options) {
  ...
  if (!fs.existsSync(config.publicReportsDir)) {
    fs.mkdirSync(config.publicReportsDir, { recursive: true });
  }

  // Save JSON report
  const jsonPath = path.join(config.reportsDir, "bundle-size-report.json");
  const publicJsonPath = path.join(
    config.publicReportsDir,
  ...
  logger.verbose(`JSON report copied to: ${publicJsonPath}`);
  ...
  if (!options.json) {
    ...
    logger.verbose(`HTML report copied to: ${publicHtmlPath}`);
