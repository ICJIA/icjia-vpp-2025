...
class SitemapGenerator {
  ...
  async loadConfiguration() {
    try {
      ...
      const sitemapConfigPath = path.join(process.cwd(), 'config/sitemap.config.json');
      const sitemapConfigContent = await fs.readFile(sitemapConfigPath, 'utf-8');
      this.config = JSON.parse(sitemapConfigContent);

      // Load site configuration for routes
      const siteConfigPath = path.join(process.cwd(), 'config/routes.config.json');
  ...
  shouldExcludeRoute(route) {
    ...
    if (route.type === 'content' && route.source) {
      try {
        ...
        if (frontmatter[exclusions.frontmatterKey] === exclusions.frontmatterValue) {
          return true;
        }
      } catch (error) {
  ...
  async processRoutes() {
    ...
    for (const route of allRoutes) {
      ...
      const fullUrl = this.config.sitemap.baseUrl.replace(/\/$/, '') + sanitizedPath;

      // Check for duplicates
      if (processedRoutes.has(sanitizedPath)) {
        this.stats.duplicatesRemoved++;
        this.logger?.addToGroup('warning', `Duplicate route removed: ${sanitizedPath}`);
        continue;
      }
    ...
    this.logger?.info(`📊 Route processing complete: ${this.stats.includedRoutes} included, ${this.stats.excludedRoutes} excluded, ${this.stats.duplicatesRemoved} duplicates removed`);
  }
  ...
  validateXML(xml) {
    ...
    try {
      ...
      if (urlCount > this.config.validation.maxUrls) {
        throw new Error(`Too many URLs: ${urlCount} (max: ${this.config.validation.maxUrls})`);
      }
  ...
  async writeSitemap(xml) {
    ...
    try {
      ...
      this.logger?.error(`❌ Failed to write sitemap to ${outputPath}:`, error.message);
      throw error;
    }
  }
