...
| **Branch-scoped Netlify preview** | Cutover flips `netlify.toml` `base` to `astro/` at the end (§17); nothing documented for previewing mid-migration | **Use a branch-context override so the migration branch deploys Astro while production keeps building the legacy app:** add `[context."<migration-branch>"]` with `base = "astro/"`, a pnpm `command`, and `publish = "astro/dist"` to the root `netlify.toml`. The production/main context is untouched → zero risk to the live site. Gives a real-URL deploy preview for the per-phase screenshot-diff + lightcap gate throughout the migration. Promote the override to the default `[build]` at cutover. |
